...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const statics = ['', '/about', '/services', '/jobs', '/employers', '/vendor-registration', '/blog', '/careers', '/contact', '/faqs', '/privacy', '/terms'].map(p => ({ url: `${APP}${p}`, changeFrequency: 'weekly' as const, priority: p === '' ? 1 : 0.7 }));
  return [
    ...statics,
