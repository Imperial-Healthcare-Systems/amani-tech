...
const NAVY = '#0B2A5F', NAVY_MID = '#134087', BLUE = '#0091FF', BLUE_LIGHT = '#5CB8FF', LAV = '#8BA7E8';

function Defs({ id }: { id: string }) {
  return (
    <defs>
      <linearGradient id={`${id}-glass`} x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stopColor="#FFFFFF" stopOpacity=".95" /><stop offset="1" stopColor="#DCEFFF" stopOpacity=".8" />
      </linearGradient>
      ...
      <linearGradient id={`${id}-lav`} x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stopColor="#C7D9FF" /><stop offset="1" stopColor={LAV} />
      </linearGradient>
...
export function WhyArt({ name }: { name: string }) {
  ...
  if (name === 'growth') return (
    <svg {...box}><Defs id={id} />
      ...
      <g className={s.floatSlow}>
        ...
        <path d="M60 104 L112 78 L164 104 L112 130 Z" fill={`url(#${id}-glass)`} stroke="#C9E6FF" strokeWidth="1" opacity=".9" />
      </g>
      <g className={s.floatFast}>
        <rect x="150" y="34" width="56" height="44" rx="9" fill="#fff" stroke="#C9E6FF" />
        ...
        <rect x="158" y="42" width="18" height="3" rx="1.5" fill="#C9E6FF" />
      </g>
      <g className={s.floatFast} style={{ animationDelay: '.8s' }}>
        <rect x="30" y="52" width="30" height="26" rx="7" fill="#fff" stroke="#C9E6FF" />
    ...
    <svg {...box}><Defs id={id} />
      ...
      <g className={s.floatSlow}>
        {[[66, 76, 15, `url(#${id}-glass)`], [136, 74, 15, `url(#${id}-lav)`]].map(([cx, cy, r, fill], i) => (
          <g key={i}>
            <circle cx={cx} cy={cy} r={r} fill={fill as string} stroke="#C9E6FF" />
            <path d={`M${Number(cx) - 21} 120 a21 21 0 0 1 42 0 Z`} fill={fill as string} stroke="#C9E6FF" />
          </g>
        ))}
        <circle cx="101" cy="62" r="19" fill={`url(#${id}-deep)`} />
        <path d="M76 120 a25 25 0 0 1 50 0 Z" fill={`url(#${id}-deep)`} />
      </g>
      <g stroke="#B9DEFF" strokeWidth="1.4" strokeDasharray="3 4" fill="none">
        <path className={s.dash} d="M78 68 L92 62" /><path className={s.dash} d="M124 66 L138 70" />
      </g>
      <g className={s.floatFast}>
        <rect x="150" y="38" width="66" height="34" rx="9" fill="#fff" stroke="#C9E6FF" />
        <circle cx="166" cy="55" r="8" fill={`url(#${id}-blue)`} />
        <rect x="180" y="49" width="28" height="3.5" rx="1.75" fill="#C9E6FF" /><rect x="180" y="57" width="18" height="3.5" rx="1.75" fill="#DCEFFF" />
      </g>
  ...
  if (name === 'outcomes') return (
    <svg {...box}><Defs id={id} />
      ...
      <g className={s.floatSlow}>
        <rect x="52" y="26" width="108" height="92" rx="12" fill={`url(#${id}-glass)`} stroke="#C9E6FF" />
        {[0, 1, 2].map(i => (
          <g key={i} className={s.tick} style={{ animationDelay: `${i * .45}s` }}>
            ...
            <rect x="90" y={47 + i * 24} width={i === 1 ? 48 : 56} height="4" rx="2" fill="#C9E6FF" />
            <rect x="90" y={55 + i * 24} width="32" height="4" rx="2" fill="#E0F1FF" />
          </g>
        ))}
      </g>
      <g className={s.floatFast}>
  ...
  return (
    <svg {...box}><Defs id={id} />
      ...
      <g className={s.floatSlow}>
        ...
        <path d="M44 114 h148 l-9 10 h-130 Z" fill={`url(#${id}-glass)`} stroke="#C9E6FF" />
      </g>
      <g stroke="#B9DEFF" strokeWidth="1.4" strokeDasharray="3 4" fill="none">
      ...
      {[[28, 44], [198, 36], [206, 84]].map(([x, y], i) => (
        <g key={i} className={s.floatFast} style={{ animationDelay: `${i * .5}s` }}>
          <rect x={x - 3} y={y - 3} width="30" height="30" rx="9" fill="#fff" stroke="#C9E6FF" />
