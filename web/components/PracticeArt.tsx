...
const NAVY = '#141A4A', NAVY_MID = '#242D78', BLUE = '#2563EB', BLUE_LIGHT = '#60A5FA', VIOLET = '#7C3AED';

function Defs({ id }: { id: string }) {
  return (
    <defs>
      <linearGradient id={`${id}-face`} x1="0" y1="0" x2="0" y2="1">
        <stop offset="0" stopColor="#FFFFFF" /><stop offset="1" stopColor="#E8EFFF" />
      </linearGradient>
...
const Tile = ({ x, y, w = 46, h = 46, id, delay = 0, children }: { x: Len; y: Len; w?: Len; h?: Len; id: string; delay?: number; children?: React.ReactNode }) => (
  <g className={s.float} style={{ animationDelay: `${delay}s` }}>
    <rect x={x} y={y} width={w} height={h} rx="11" fill={`url(#${id}-face)`} stroke="#C9D8FF" />
    {children}
  </g>
);
...
export function PracticeArt({ name }: { name: string }) {
  ...
  if (name === 'ai') return (
    <svg {...common}><Defs id={id} />
      ...
      {/* isometric platform */}
      <path d="M210 196 L118 150 L210 104 L302 150 Z" fill={`url(#${id}-face)`} stroke="#C9D8FF" />
      <path d="M118 150 L210 196 L210 208 L118 162 Z" fill="#DCE6FF" />
      <path d="M302 150 L210 196 L210 208 L302 162 Z" fill="#C9D8FF" />
      {/* chip */}
      <g className={s.rise}>
      ...
      {/* connectors + nodes */}
      <g stroke="#B9CCFF" strokeWidth="1.5" strokeDasharray="4 5" fill="none">
        <path d="M142 90 L188 106" /><path d="M300 74 L252 100" /><path d="M318 152 L268 134" />
      </g>
      <Tile x="104" y="60" id={id} delay={0}>
        <rect x="116" y="74" width="22" height="3" rx="1.5" fill={BLUE} /><rect x="116" y="82" width="16" height="3" rx="1.5" fill="#9DB8F5" /><rect x="116" y="90" width="20" height="3" rx="1.5" fill="#9DB8F5" />
      </Tile>
    ...
    <svg {...common}><Defs id={id} />
      ...
      {[0, 1].map(i => (
        <g key={i} transform={`translate(0 ${i * -22})`}>
          <path d="M210 208 L142 172 L210 136 L278 172 Z" fill={`url(#${id}-face)`} stroke="#C9D8FF" />
          <path d="M142 172 L210 208 L210 220 L142 184 Z" fill="#D5E1FF" />
          <path d="M278 172 L210 208 L210 220 L278 184 Z" fill="#BFD2FF" />
      ...
      <Tile x="72" y="106" id={id} delay={.3}><path d="M84 132 h22 M84 138 h14" stroke={BLUE} strokeWidth="3" strokeLinecap="round" /><circle cx="95" cy="120" r="6" fill="none" stroke="#9DB8F5" strokeWidth="2.5" /></Tile>
  ...
  if (name === 'software') return (
    <svg {...common}><Defs id={id} />
      ...
      <g className={s.rise}>
        ...
        <path d="M104 170 h212 l-10 14 h-192 Z" fill="#DCE6FF" stroke="#C9D8FF" />
      </g>
      {/* phone */}
      <g className={s.float} style={{ animationDelay: '.4s' }}>
        <rect x="252" y="112" width="58" height="96" rx="12" fill="#fff" stroke="#C9D8FF" />
        <rect x="260" y="124" width="42" height="26" rx="6" fill={`url(#${id}-blue)`} />
        <rect x="260" y="158" width="42" height="5" rx="2.5" fill="#D5E1FF" />
        <rect x="260" y="170" width="30" height="5" rx="2.5" fill="#D5E1FF" />
        <rect x="260" y="184" width="42" height="12" rx="6" fill="#EFF4FF" stroke="#C9D8FF" />
      </g>
      ...
      <Tile x="326" y="140" id={id} delay={.6}><path d="M338 164 h18 M338 172 h10" stroke={BLUE} strokeWidth="2.5" strokeLinecap="round" /><circle cx="349" cy="156" r="5" fill="none" stroke="#9DB8F5" strokeWidth="2" /></Tile>
  ...
  return (
    <svg {...common}><Defs id={id} />
      ...
      <g className={s.float}>
        {[0, 1, 2].map(i => (
          <g key={i} transform={`translate(0 ${i * 26})`}>
            <ellipse cx="120" cy="120" rx="40" ry="14" fill={`url(#${id}-face)`} stroke="#C9D8FF" />
            <path d="M80 120 v18 a40 14 0 0 0 80 0 v-18" fill="#E8EFFF" stroke="#C9D8FF" />
          </g>
        ))}
      </g>
      {/* dashboard panel */}
      <g className={s.rise}>
        <rect x="186" y="74" width="168" height="116" rx="12" fill="#fff" stroke="#C9D8FF" />
        <rect x="200" y="88" width="56" height="5" rx="2.5" fill="#9DB8F5" />
      ...
      <g stroke="#B9CCFF" strokeWidth="1.5" strokeDasharray="4 5" fill="none"><path d="M162 132 L186 126" /></g>
