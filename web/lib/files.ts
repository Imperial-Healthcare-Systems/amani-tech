...
export async function storeFile(file: File | null, bucket: 'resumes' | 'documents' | 'media', prefix: string, kind: 'doc' | 'image' = 'doc'): Promise<{ path: string; name: string } | null> {
  ...
  if (file.size > MAX_MB * 1024 * 1024) throw new Error(`File is too large. Maximum size is ${MAX_MB} MB.`);
...
export async function signedUrl(bucket: 'resumes' | 'documents', path: string, seconds = 300) {
